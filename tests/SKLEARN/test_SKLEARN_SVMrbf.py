...
class test_SKLEARN_SVMrbf(unittest.TestCase):

    params = [("{'C':1.0,'kernel':'rbf','gamma':0.01,}", {'C':1.0,'kernel':'rbf','gamma':0.01,}),
             ]
    ...
    def test_UniformSplit(self, name, kwargs):
        for iter in range(1,1+1):
            ...
            try:
                ...
                prediction_file = os.path.join(main_dir, "predictions", "pred_SKLEARN_SVMrbf_UniformSplit_" + str(iter) + ".csv")
                ...
                print("Saving the predictions of UniformSplit for SKLEARN_SVMrbf failed: ", e)
            ...
            try:
              main_dir = Path(__file__).resolve().parents[2]
              prediction_file_training_as_test = os.path.join(main_dir, "predictions", "pred_SKLEARN_SVMrbf_UniformSplit_TrainingAsTest_" + str(iter) + ".csv")
              pred_df = pd.DataFrame()
              ...
              print("Saving the predictions of UniformSplit with TrainingAsTest for SKLEARN_SVMrbf failed: ", e)
    @parameterized.expand(params)
    @timeout(21600)
    def test_RandomNumericSplit(self, name, kwargs):
        for iter in range(1,1+1):
            ...
            try:
                main_dir = Path(__file__).resolve().parents[2]
                prediction_file = os.path.join(main_dir, "predictions", "pred_SKLEARN_SVMrbf_RandomNumericSplit_" + str(iter) + ".csv")
                pred_df = pd.DataFrame()
                pred_df['actual'] = testdata_df.classAtt
                pred_df['prediction'] = predicted_label
                pred_df['prob_0'] = pred_prob[:,0]
                ...
                print("Saving the predictions of RandomNumericSplit for SKLEARN_SVMrbf failed: ", e)
            ...
            try:
              main_dir = Path(__file__).resolve().parents[2]
              prediction_file_training_as_test = os.path.join(main_dir, "predictions", "pred_SKLEARN_SVMrbf_RandomNumericSplit_TrainingAsTest_" + str(iter) + ".csv")
              pred_df = pd.DataFrame()
              pred_df['actual'] = data_df.classAtt
              pred_df['prediction'] = predicted_label_training_as_test
              pred_df['prob_0'] = pred_prob_training_as_test[:,0]
              ...
              print("Saving the predictions of RandomNumericSplit with TrainingAsTest for SKLEARN_SVMrbf failed: ", e)
    @parameterized.expand(params)
    @timeout(21600)
    def test_BreastCancer(self, name, kwargs):
        for iter in range(1,1+1):
            ...
            try:
                main_dir = Path(__file__).resolve().parents[2]
                prediction_file = os.path.join(main_dir, "predictions", "pred_SKLEARN_SVMrbf_BreastCancer_" + str(iter) + ".csv")
                pred_df = pd.DataFrame()
                pred_df['actual'] = testdata_df.classAtt
                pred_df['prediction'] = predicted_label
                pred_df['prob_0'] = pred_prob[:,0]
                ...
                print("Saving the predictions of BreastCancer for SKLEARN_SVMrbf failed: ", e)
            ...
            try:
              main_dir = Path(__file__).resolve().parents[2]
              prediction_file_training_as_test = os.path.join(main_dir, "predictions", "pred_SKLEARN_SVMrbf_BreastCancer_TrainingAsTest_" + str(iter) + ".csv")
              pred_df = pd.DataFrame()
              pred_df['actual'] = data_df.classAtt
              pred_df['prediction'] = predicted_label_training_as_test
              pred_df['prob_0'] = pred_prob_training_as_test[:,0]
              ...
              print("Saving the predictions of BreastCancer with TrainingAsTest for SKLEARN_SVMrbf failed: ", e)
    @parameterized.expand(params)
    @timeout(21600)
    def test_BreastCancerZNorm(self, name, kwargs):
        for iter in range(1,1+1):
            ...
            try:
                main_dir = Path(__file__).resolve().parents[2]
                prediction_file = os.path.join(main_dir, "predictions", "pred_SKLEARN_SVMrbf_BreastCancerZNorm_" + str(iter) + ".csv")
                pred_df = pd.DataFrame()
                pred_df['actual'] = testdata_df.classAtt
                pred_df['prediction'] = predicted_label
                pred_df['prob_0'] = pred_prob[:,0]
                ...
                print("Saving the predictions of BreastCancerZNorm for SKLEARN_SVMrbf failed: ", e)
            ...
            try:
              main_dir = Path(__file__).resolve().parents[2]
              prediction_file_training_as_test = os.path.join(main_dir, "predictions", "pred_SKLEARN_SVMrbf_BreastCancerZNorm_TrainingAsTest_" + str(iter) + ".csv")
              pred_df = pd.DataFrame()
              pred_df['actual'] = data_df.classAtt
              pred_df['prediction'] = predicted_label_training_as_test
              pred_df['prob_0'] = pred_prob_training_as_test[:,0]
              ...
              print("Saving the predictions of BreastCancerZNorm with TrainingAsTest for SKLEARN_SVMrbf failed: ", e)
    @parameterized.expand(params)
    @timeout(21600)
    def test_BreastCancerMinMaxNorm(self, name, kwargs):
        for iter in range(1,1+1):
            ...
            try:
                main_dir = Path(__file__).resolve().parents[2]
                prediction_file = os.path.join(main_dir, "predictions", "pred_SKLEARN_SVMrbf_BreastCancerMinMaxNorm_" + str(iter) + ".csv")
                pred_df = pd.DataFrame()
                pred_df['actual'] = testdata_df.classAtt
                pred_df['prediction'] = predicted_label
                pred_df['prob_0'] = pred_prob[:,0]
                ...
                print("Saving the predictions of BreastCancerMinMaxNorm for SKLEARN_SVMrbf failed: ", e)
            ...
            try:
              main_dir = Path(__file__).resolve().parents[2]
              prediction_file_training_as_test = os.path.join(main_dir, "predictions", "pred_SKLEARN_SVMrbf_BreastCancerMinMaxNorm_TrainingAsTest_" + str(iter) + ".csv")
              pred_df = pd.DataFrame()
              pred_df['actual'] = data_df.classAtt
              pred_df['prediction'] = predicted_label_training_as_test
              pred_df['prob_0'] = pred_prob_training_as_test[:,0]
              ...
              print("Saving the predictions of BreastCancerMinMaxNorm with TrainingAsTest for SKLEARN_SVMrbf failed: ", e)
    @parameterized.expand(params)
    @timeout(21600)
    def test_Wine(self, name, kwargs):
        for iter in range(1,1+1):
            ...
            try:
                main_dir = Path(__file__).resolve().parents[2]
                prediction_file = os.path.join(main_dir, "predictions", "pred_SKLEARN_SVMrbf_Wine_" + str(iter) + ".csv")
                pred_df = pd.DataFrame()
                pred_df['actual'] = testdata_df.classAtt
                pred_df['prediction'] = predicted_label
                pred_df['prob_0'] = pred_prob[:,0]
                ...
                print("Saving the predictions of Wine for SKLEARN_SVMrbf failed: ", e)
            ...
            try:
              main_dir = Path(__file__).resolve().parents[2]
              prediction_file_training_as_test = os.path.join(main_dir, "predictions", "pred_SKLEARN_SVMrbf_Wine_TrainingAsTest_" + str(iter) + ".csv")
              pred_df = pd.DataFrame()
              pred_df['actual'] = data_df.classAtt
              pred_df['prediction'] = predicted_label_training_as_test
              pred_df['prob_0'] = pred_prob_training_as_test[:,0]
              ...
              print("Saving the predictions of Wine with TrainingAsTest for SKLEARN_SVMrbf failed: ", e)
    @parameterized.expand(params)
    @timeout(21600)
    def test_WineZNorm(self, name, kwargs):
        for iter in range(1,1+1):
            ...
            try:
                main_dir = Path(__file__).resolve().parents[2]
                prediction_file = os.path.join(main_dir, "predictions", "pred_SKLEARN_SVMrbf_WineZNorm_" + str(iter) + ".csv")
                pred_df = pd.DataFrame()
                pred_df['actual'] = testdata_df.classAtt
                pred_df['prediction'] = predicted_label
                pred_df['prob_0'] = pred_prob[:,0]
                ...
                print("Saving the predictions of WineZNorm for SKLEARN_SVMrbf failed: ", e)
            ...
            try:
              main_dir = Path(__file__).resolve().parents[2]
              prediction_file_training_as_test = os.path.join(main_dir, "predictions", "pred_SKLEARN_SVMrbf_WineZNorm_TrainingAsTest_" + str(iter) + ".csv")
              pred_df = pd.DataFrame()
              pred_df['actual'] = data_df.classAtt
              pred_df['prediction'] = predicted_label_training_as_test
              pred_df['prob_0'] = pred_prob_training_as_test[:,0]
              ...
              print("Saving the predictions of WineZNorm with TrainingAsTest for SKLEARN_SVMrbf failed: ", e)
    @parameterized.expand(params)
    @timeout(21600)
    def test_WineMinMaxNorm(self, name, kwargs):
        for iter in range(1,1+1):
            ...
            try:
                main_dir = Path(__file__).resolve().parents[2]
                prediction_file = os.path.join(main_dir, "predictions", "pred_SKLEARN_SVMrbf_WineMinMaxNorm_" + str(iter) + ".csv")
                pred_df = pd.DataFrame()
                pred_df['actual'] = testdata_df.classAtt
                pred_df['prediction'] = predicted_label
                pred_df['prob_0'] = pred_prob[:,0]
                ...
                print("Saving the predictions of WineMinMaxNorm for SKLEARN_SVMrbf failed: ", e)
            ...
            try:
              main_dir = Path(__file__).resolve().parents[2]
              prediction_file_training_as_test = os.path.join(main_dir, "predictions", "pred_SKLEARN_SVMrbf_WineMinMaxNorm_TrainingAsTest_" + str(iter) + ".csv")
              pred_df = pd.DataFrame()
              pred_df['actual'] = data_df.classAtt
              pred_df['prediction'] = predicted_label_training_as_test
              pred_df['prob_0'] = pred_prob_training_as_test[:,0]
              ...
              print("Saving the predictions of WineMinMaxNorm with TrainingAsTest for SKLEARN_SVMrbf failed: ", e)
