...
class test_SKLEARN_LRlasso(unittest.TestCase):

    params = [("{'C':5.0,'penalty':'l1','max_iter':10000,'solver':'liblinear',}", {'C':5.0,'penalty':'l1','max_iter':10000,'solver':'liblinear',}),
             ]
    ...
    def test_UniformSplit(self, name, kwargs):
        for iter in range(1,1+1):
            ...
            try:
                ...
                prediction_file = os.path.join(main_dir, "predictions", "pred_SKLEARN_LRlasso_UniformSplit_" + str(iter) + ".csv")
                ...
                print("Saving the predictions of UniformSplit for SKLEARN_LRlasso failed: ", e)
            ...
            try:
              main_dir = Path(__file__).resolve().parents[2]
              prediction_file_training_as_test = os.path.join(main_dir, "predictions", "pred_SKLEARN_LRlasso_UniformSplit_TrainingAsTest_" + str(iter) + ".csv")
              pred_df = pd.DataFrame()
              ...
              print("Saving the predictions of UniformSplit with TrainingAsTest for SKLEARN_LRlasso failed: ", e)
    @parameterized.expand(params)
    @timeout(21600)
    def test_RandomNumericSplit(self, name, kwargs):
        for iter in range(1,1+1):
            ...
            try:
                main_dir = Path(__file__).resolve().parents[2]
                prediction_file = os.path.join(main_dir, "predictions", "pred_SKLEARN_LRlasso_RandomNumericSplit_" + str(iter) + ".csv")
                pred_df = pd.DataFrame()
                pred_df['actual'] = testdata_df.classAtt
                pred_df['prediction'] = predicted_label
                pred_df['prob_0'] = pred_prob[:,0]
                ...
                print("Saving the predictions of RandomNumericSplit for SKLEARN_LRlasso failed: ", e)
            ...
            try:
              main_dir = Path(__file__).resolve().parents[2]
              prediction_file_training_as_test = os.path.join(main_dir, "predictions", "pred_SKLEARN_LRlasso_RandomNumericSplit_TrainingAsTest_" + str(iter) + ".csv")
              pred_df = pd.DataFrame()
              pred_df['actual'] = data_df.classAtt
              pred_df['prediction'] = predicted_label_training_as_test
              pred_df['prob_0'] = pred_prob_training_as_test[:,0]
              ...
              print("Saving the predictions of RandomNumericSplit with TrainingAsTest for SKLEARN_LRlasso failed: ", e)
    @parameterized.expand(params)
    @timeout(21600)
    def test_BreastCancer(self, name, kwargs):
        for iter in range(1,1+1):
            ...
            try:
                main_dir = Path(__file__).resolve().parents[2]
                prediction_file = os.path.join(main_dir, "predictions", "pred_SKLEARN_LRlasso_BreastCancer_" + str(iter) + ".csv")
                pred_df = pd.DataFrame()
                pred_df['actual'] = testdata_df.classAtt
                pred_df['prediction'] = predicted_label
                pred_df['prob_0'] = pred_prob[:,0]
                ...
                print("Saving the predictions of BreastCancer for SKLEARN_LRlasso failed: ", e)
            ...
            try:
              main_dir = Path(__file__).resolve().parents[2]
              prediction_file_training_as_test = os.path.join(main_dir, "predictions", "pred_SKLEARN_LRlasso_BreastCancer_TrainingAsTest_" + str(iter) + ".csv")
              pred_df = pd.DataFrame()
              pred_df['actual'] = data_df.classAtt
              pred_df['prediction'] = predicted_label_training_as_test
              pred_df['prob_0'] = pred_prob_training_as_test[:,0]
              ...
              print("Saving the predictions of BreastCancer with TrainingAsTest for SKLEARN_LRlasso failed: ", e)
    @parameterized.expand(params)
    @timeout(21600)
    def test_BreastCancerZNorm(self, name, kwargs):
        for iter in range(1,1+1):
            ...
            try:
                main_dir = Path(__file__).resolve().parents[2]
                prediction_file = os.path.join(main_dir, "predictions", "pred_SKLEARN_LRlasso_BreastCancerZNorm_" + str(iter) + ".csv")
                pred_df = pd.DataFrame()
                pred_df['actual'] = testdata_df.classAtt
                pred_df['prediction'] = predicted_label
                pred_df['prob_0'] = pred_prob[:,0]
                ...
                print("Saving the predictions of BreastCancerZNorm for SKLEARN_LRlasso failed: ", e)
            ...
            try:
              main_dir = Path(__file__).resolve().parents[2]
              prediction_file_training_as_test = os.path.join(main_dir, "predictions", "pred_SKLEARN_LRlasso_BreastCancerZNorm_TrainingAsTest_" + str(iter) + ".csv")
              pred_df = pd.DataFrame()
              pred_df['actual'] = data_df.classAtt
              pred_df['prediction'] = predicted_label_training_as_test
              pred_df['prob_0'] = pred_prob_training_as_test[:,0]
              ...
              print("Saving the predictions of BreastCancerZNorm with TrainingAsTest for SKLEARN_LRlasso failed: ", e)
    @parameterized.expand(params)
    @timeout(21600)
    def test_BreastCancerMinMaxNorm(self, name, kwargs):
        for iter in range(1,1+1):
            ...
            try:
                main_dir = Path(__file__).resolve().parents[2]
                prediction_file = os.path.join(main_dir, "predictions", "pred_SKLEARN_LRlasso_BreastCancerMinMaxNorm_" + str(iter) + ".csv")
                pred_df = pd.DataFrame()
                pred_df['actual'] = testdata_df.classAtt
                pred_df['prediction'] = predicted_label
                pred_df['prob_0'] = pred_prob[:,0]
                ...
                print("Saving the predictions of BreastCancerMinMaxNorm for SKLEARN_LRlasso failed: ", e)
            ...
            try:
              main_dir = Path(__file__).resolve().parents[2]
              prediction_file_training_as_test = os.path.join(main_dir, "predictions", "pred_SKLEARN_LRlasso_BreastCancerMinMaxNorm_TrainingAsTest_" + str(iter) + ".csv")
              pred_df = pd.DataFrame()
              pred_df['actual'] = data_df.classAtt
              pred_df['prediction'] = predicted_label_training_as_test
              pred_df['prob_0'] = pred_prob_training_as_test[:,0]
              ...
              print("Saving the predictions of BreastCancerMinMaxNorm with TrainingAsTest for SKLEARN_LRlasso failed: ", e)
    @parameterized.expand(params)
    @timeout(21600)
    def test_Wine(self, name, kwargs):
        for iter in range(1,1+1):
            ...
            try:
                main_dir = Path(__file__).resolve().parents[2]
                prediction_file = os.path.join(main_dir, "predictions", "pred_SKLEARN_LRlasso_Wine_" + str(iter) + ".csv")
                pred_df = pd.DataFrame()
                pred_df['actual'] = testdata_df.classAtt
                pred_df['prediction'] = predicted_label
                pred_df['prob_0'] = pred_prob[:,0]
                ...
                print("Saving the predictions of Wine for SKLEARN_LRlasso failed: ", e)
            ...
            try:
              main_dir = Path(__file__).resolve().parents[2]
              prediction_file_training_as_test = os.path.join(main_dir, "predictions", "pred_SKLEARN_LRlasso_Wine_TrainingAsTest_" + str(iter) + ".csv")
              pred_df = pd.DataFrame()
              pred_df['actual'] = data_df.classAtt
              pred_df['prediction'] = predicted_label_training_as_test
              pred_df['prob_0'] = pred_prob_training_as_test[:,0]
              ...
              print("Saving the predictions of Wine with TrainingAsTest for SKLEARN_LRlasso failed: ", e)
    @parameterized.expand(params)
    @timeout(21600)
    def test_WineZNorm(self, name, kwargs):
        for iter in range(1,1+1):
            ...
            try:
                main_dir = Path(__file__).resolve().parents[2]
                prediction_file = os.path.join(main_dir, "predictions", "pred_SKLEARN_LRlasso_WineZNorm_" + str(iter) + ".csv")
                pred_df = pd.DataFrame()
                pred_df['actual'] = testdata_df.classAtt
                pred_df['prediction'] = predicted_label
                pred_df['prob_0'] = pred_prob[:,0]
                ...
                print("Saving the predictions of WineZNorm for SKLEARN_LRlasso failed: ", e)
            ...
            try:
              main_dir = Path(__file__).resolve().parents[2]
              prediction_file_training_as_test = os.path.join(main_dir, "predictions", "pred_SKLEARN_LRlasso_WineZNorm_TrainingAsTest_" + str(iter) + ".csv")
              pred_df = pd.DataFrame()
              pred_df['actual'] = data_df.classAtt
              pred_df['prediction'] = predicted_label_training_as_test
              pred_df['prob_0'] = pred_prob_training_as_test[:,0]
              ...
              print("Saving the predictions of WineZNorm with TrainingAsTest for SKLEARN_LRlasso failed: ", e)
    @parameterized.expand(params)
    @timeout(21600)
    def test_WineMinMaxNorm(self, name, kwargs):
        for iter in range(1,1+1):
            ...
            try:
                main_dir = Path(__file__).resolve().parents[2]
                prediction_file = os.path.join(main_dir, "predictions", "pred_SKLEARN_LRlasso_WineMinMaxNorm_" + str(iter) + ".csv")
                pred_df = pd.DataFrame()
                pred_df['actual'] = testdata_df.classAtt
                pred_df['prediction'] = predicted_label
                pred_df['prob_0'] = pred_prob[:,0]
                ...
                print("Saving the predictions of WineMinMaxNorm for SKLEARN_LRlasso failed: ", e)
            ...
            try:
              main_dir = Path(__file__).resolve().parents[2]
              prediction_file_training_as_test = os.path.join(main_dir, "predictions", "pred_SKLEARN_LRlasso_WineMinMaxNorm_TrainingAsTest_" + str(iter) + ".csv")
              pred_df = pd.DataFrame()
              pred_df['actual'] = data_df.classAtt
              pred_df['prediction'] = predicted_label_training_as_test
              pred_df['prob_0'] = pred_prob_training_as_test[:,0]
              ...
              print("Saving the predictions of WineMinMaxNorm with TrainingAsTest for SKLEARN_LRlasso failed: ", e)
