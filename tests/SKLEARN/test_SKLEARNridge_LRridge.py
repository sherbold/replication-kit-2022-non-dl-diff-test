...
class test_SKLEARNridge_LRridge(unittest.TestCase):

    params = [("{'alpha':0.1,'max_iter':10000,}", {'alpha':0.1,'max_iter':10000,}),
             ]
    ...
    def test_UniformSplit(self, name, kwargs):
        for iter in range(1,1+1):
            ...
            try:
                ...
                prediction_file = os.path.join(main_dir, "predictions", "pred_SKLEARNridge_LRridge_UniformSplit_" + str(iter) + ".csv")
                ...
                print("Saving the predictions of UniformSplit for SKLEARNridge_LRridge failed: ", e)
            ...
            try:
              main_dir = Path(__file__).resolve().parents[2]
              prediction_file_training_as_test = os.path.join(main_dir, "predictions", "pred_SKLEARNridge_LRridge_UniformSplit_TrainingAsTest_" + str(iter) + ".csv")
              pred_df = pd.DataFrame()
              ...
              print("Saving the predictions of UniformSplit with TrainingAsTest for SKLEARNridge_LRridge failed: ", e)
    @parameterized.expand(params)
    @timeout(21600)
    def test_RandomNumericSplit(self, name, kwargs):
        for iter in range(1,1+1):
            ...
            try:
                main_dir = Path(__file__).resolve().parents[2]
                prediction_file = os.path.join(main_dir, "predictions", "pred_SKLEARNridge_LRridge_RandomNumericSplit_" + str(iter) + ".csv")
                pred_df = pd.DataFrame()
                pred_df['actual'] = testdata_df.classAtt
                pred_df['prediction'] = predicted_label
                pred_df['prob_0'] = pred_prob[:,0]
                ...
                print("Saving the predictions of RandomNumericSplit for SKLEARNridge_LRridge failed: ", e)
            ...
            try:
              main_dir = Path(__file__).resolve().parents[2]
              prediction_file_training_as_test = os.path.join(main_dir, "predictions", "pred_SKLEARNridge_LRridge_RandomNumericSplit_TrainingAsTest_" + str(iter) + ".csv")
              pred_df = pd.DataFrame()
              pred_df['actual'] = data_df.classAtt
              pred_df['prediction'] = predicted_label_training_as_test
              pred_df['prob_0'] = pred_prob_training_as_test[:,0]
              ...
              print("Saving the predictions of RandomNumericSplit with TrainingAsTest for SKLEARNridge_LRridge failed: ", e)
    @parameterized.expand(params)
    @timeout(21600)
    def test_BreastCancer(self, name, kwargs):
        for iter in range(1,1+1):
            ...
            try:
                main_dir = Path(__file__).resolve().parents[2]
                prediction_file = os.path.join(main_dir, "predictions", "pred_SKLEARNridge_LRridge_BreastCancer_" + str(iter) + ".csv")
                pred_df = pd.DataFrame()
                pred_df['actual'] = testdata_df.classAtt
                pred_df['prediction'] = predicted_label
                pred_df['prob_0'] = pred_prob[:,0]
                ...
                print("Saving the predictions of BreastCancer for SKLEARNridge_LRridge failed: ", e)
            ...
            try:
              main_dir = Path(__file__).resolve().parents[2]
              prediction_file_training_as_test = os.path.join(main_dir, "predictions", "pred_SKLEARNridge_LRridge_BreastCancer_TrainingAsTest_" + str(iter) + ".csv")
              pred_df = pd.DataFrame()
              pred_df['actual'] = data_df.classAtt
              pred_df['prediction'] = predicted_label_training_as_test
              pred_df['prob_0'] = pred_prob_training_as_test[:,0]
              ...
              print("Saving the predictions of BreastCancer with TrainingAsTest for SKLEARNridge_LRridge failed: ", e)
    @parameterized.expand(params)
    @timeout(21600)
    def test_BreastCancerZNorm(self, name, kwargs):
        for iter in range(1,1+1):
            ...
            try:
                main_dir = Path(__file__).resolve().parents[2]
                prediction_file = os.path.join(main_dir, "predictions", "pred_SKLEARNridge_LRridge_BreastCancerZNorm_" + str(iter) + ".csv")
                pred_df = pd.DataFrame()
                pred_df['actual'] = testdata_df.classAtt
                pred_df['prediction'] = predicted_label
                pred_df['prob_0'] = pred_prob[:,0]
                ...
                print("Saving the predictions of BreastCancerZNorm for SKLEARNridge_LRridge failed: ", e)
            ...
            try:
              main_dir = Path(__file__).resolve().parents[2]
              prediction_file_training_as_test = os.path.join(main_dir, "predictions", "pred_SKLEARNridge_LRridge_BreastCancerZNorm_TrainingAsTest_" + str(iter) + ".csv")
              pred_df = pd.DataFrame()
              pred_df['actual'] = data_df.classAtt
              pred_df['prediction'] = predicted_label_training_as_test
              pred_df['prob_0'] = pred_prob_training_as_test[:,0]
              ...
              print("Saving the predictions of BreastCancerZNorm with TrainingAsTest for SKLEARNridge_LRridge failed: ", e)
    @parameterized.expand(params)
    @timeout(21600)
    def test_BreastCancerMinMaxNorm(self, name, kwargs):
        for iter in range(1,1+1):
            ...
            try:
                main_dir = Path(__file__).resolve().parents[2]
                prediction_file = os.path.join(main_dir, "predictions", "pred_SKLEARNridge_LRridge_BreastCancerMinMaxNorm_" + str(iter) + ".csv")
                pred_df = pd.DataFrame()
                pred_df['actual'] = testdata_df.classAtt
                pred_df['prediction'] = predicted_label
                pred_df['prob_0'] = pred_prob[:,0]
                ...
                print("Saving the predictions of BreastCancerMinMaxNorm for SKLEARNridge_LRridge failed: ", e)
            ...
            try:
              main_dir = Path(__file__).resolve().parents[2]
              prediction_file_training_as_test = os.path.join(main_dir, "predictions", "pred_SKLEARNridge_LRridge_BreastCancerMinMaxNorm_TrainingAsTest_" + str(iter) + ".csv")
              pred_df = pd.DataFrame()
              pred_df['actual'] = data_df.classAtt
              pred_df['prediction'] = predicted_label_training_as_test
              pred_df['prob_0'] = pred_prob_training_as_test[:,0]
              ...
              print("Saving the predictions of BreastCancerMinMaxNorm with TrainingAsTest for SKLEARNridge_LRridge failed: ", e)
    @parameterized.expand(params)
    @timeout(21600)
    def test_Wine(self, name, kwargs):
        for iter in range(1,1+1):
            ...
            try:
                main_dir = Path(__file__).resolve().parents[2]
                prediction_file = os.path.join(main_dir, "predictions", "pred_SKLEARNridge_LRridge_Wine_" + str(iter) + ".csv")
                pred_df = pd.DataFrame()
                pred_df['actual'] = testdata_df.classAtt
                pred_df['prediction'] = predicted_label
                pred_df['prob_0'] = pred_prob[:,0]
                ...
                print("Saving the predictions of Wine for SKLEARNridge_LRridge failed: ", e)
            ...
            try:
              main_dir = Path(__file__).resolve().parents[2]
              prediction_file_training_as_test = os.path.join(main_dir, "predictions", "pred_SKLEARNridge_LRridge_Wine_TrainingAsTest_" + str(iter) + ".csv")
              pred_df = pd.DataFrame()
              pred_df['actual'] = data_df.classAtt
              pred_df['prediction'] = predicted_label_training_as_test
              pred_df['prob_0'] = pred_prob_training_as_test[:,0]
              ...
              print("Saving the predictions of Wine with TrainingAsTest for SKLEARNridge_LRridge failed: ", e)
    @parameterized.expand(params)
    @timeout(21600)
    def test_WineZNorm(self, name, kwargs):
        for iter in range(1,1+1):
            ...
            try:
                main_dir = Path(__file__).resolve().parents[2]
                prediction_file = os.path.join(main_dir, "predictions", "pred_SKLEARNridge_LRridge_WineZNorm_" + str(iter) + ".csv")
                pred_df = pd.DataFrame()
                pred_df['actual'] = testdata_df.classAtt
                pred_df['prediction'] = predicted_label
                pred_df['prob_0'] = pred_prob[:,0]
                ...
                print("Saving the predictions of WineZNorm for SKLEARNridge_LRridge failed: ", e)
            ...
            try:
              main_dir = Path(__file__).resolve().parents[2]
              prediction_file_training_as_test = os.path.join(main_dir, "predictions", "pred_SKLEARNridge_LRridge_WineZNorm_TrainingAsTest_" + str(iter) + ".csv")
              pred_df = pd.DataFrame()
              pred_df['actual'] = data_df.classAtt
              pred_df['prediction'] = predicted_label_training_as_test
              pred_df['prob_0'] = pred_prob_training_as_test[:,0]
              ...
              print("Saving the predictions of WineZNorm with TrainingAsTest for SKLEARNridge_LRridge failed: ", e)
    @parameterized.expand(params)
    @timeout(21600)
    def test_WineMinMaxNorm(self, name, kwargs):
        for iter in range(1,1+1):
            ...
            try:
                main_dir = Path(__file__).resolve().parents[2]
                prediction_file = os.path.join(main_dir, "predictions", "pred_SKLEARNridge_LRridge_WineMinMaxNorm_" + str(iter) + ".csv")
                pred_df = pd.DataFrame()
                pred_df['actual'] = testdata_df.classAtt
                pred_df['prediction'] = predicted_label
                pred_df['prob_0'] = pred_prob[:,0]
                ...
                print("Saving the predictions of WineMinMaxNorm for SKLEARNridge_LRridge failed: ", e)
            ...
            try:
              main_dir = Path(__file__).resolve().parents[2]
              prediction_file_training_as_test = os.path.join(main_dir, "predictions", "pred_SKLEARNridge_LRridge_WineMinMaxNorm_TrainingAsTest_" + str(iter) + ".csv")
              pred_df = pd.DataFrame()
              pred_df['actual'] = data_df.classAtt
              pred_df['prediction'] = predicted_label_training_as_test
              pred_df['prob_0'] = pred_prob_training_as_test[:,0]
              ...
              print("Saving the predictions of WineMinMaxNorm with TrainingAsTest for SKLEARNridge_LRridge failed: ", e)
