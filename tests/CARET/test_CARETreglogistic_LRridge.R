...
if(!require("LiblineaR", character.only = TRUE ))
  install.packages("LiblineaR", dependencies = TRUE)
library("LiblineaR", character.only = TRUE)

paramGrid <- c(
              "epsilon,loss,cost: 0.0001,L2_primal,10")
...
withTimeout(timeout=21600, onTimeout="error", expr={
    with_parameters_test_that("UniformSplit", test_name=paramGrid, test_params=paramGrid, code={
        for (iter in 1:1){
            ...
            csv_file <- file.path(dirname(dirname(getwd())),"predictions",paste0("pred_CARETreglogistic_LRridge_UniformSplit_",iter,".csv"))
            write.csv(x = csv_df,
                    file = csv_file,
                    row.names = FALSE)
            ...
            csv_file <- file.path(dirname(dirname(getwd())),"predictions",paste0("pred_CARETreglogistic_LRridge_UniformSplit_TrainingAsTest_",iter,".csv"))
            write.csv(x = csv_df,
                    file = csv_file,
                    row.names = FALSE)
            print(paste0("Predictions saved at: ", csv_file))
    ...
    with_parameters_test_that("RandomNumericSplit", test_name=paramGrid, test_params=paramGrid, code={
        for (iter in 1:1){
            ...
            csv_df <- cbind(actual = actual_classes,
                          ...
                          prob_0 = probabilities[,1],
                          prob_1 = probabilities[,2])
            csv_file <- file.path(dirname(dirname(getwd())),"predictions",paste0("pred_CARETreglogistic_LRridge_RandomNumericSplit_",iter,".csv"))
            write.csv(x = csv_df,
                    file = csv_file,
                    row.names = FALSE)
            print(paste0("Predictions saved at: ", csv_file))
            ...
            csv_df <- cbind(actual = actual_classes_training_as_test,
                          ...
                          prob_0 = probabilities_training_as_test[,1],
                          prob_1 = probabilities_training_as_test[,2])
            csv_file <- file.path(dirname(dirname(getwd())),"predictions",paste0("pred_CARETreglogistic_LRridge_RandomNumericSplit_TrainingAsTest_",iter,".csv"))
            write.csv(x = csv_df,
                    file = csv_file,
                    row.names = FALSE)
            print(paste0("Predictions saved at: ", csv_file))
    ...
    with_parameters_test_that("BreastCancer", test_name=paramGrid, test_params=paramGrid, code={
        for (iter in 1:1){
            ...
            csv_df <- cbind(actual = actual_classes,
                          ...
                          prob_0 = probabilities[,1],
                          prob_1 = probabilities[,2])
            csv_file <- file.path(dirname(dirname(getwd())),"predictions",paste0("pred_CARETreglogistic_LRridge_BreastCancer_",iter,".csv"))
            write.csv(x = csv_df,
                    file = csv_file,
                    row.names = FALSE)
            print(paste0("Predictions saved at: ", csv_file))
            ...
            csv_df <- cbind(actual = actual_classes_training_as_test,
                          ...
                          prob_0 = probabilities_training_as_test[,1],
                          prob_1 = probabilities_training_as_test[,2])
            csv_file <- file.path(dirname(dirname(getwd())),"predictions",paste0("pred_CARETreglogistic_LRridge_BreastCancer_TrainingAsTest_",iter,".csv"))
            write.csv(x = csv_df,
                    file = csv_file,
                    row.names = FALSE)
            print(paste0("Predictions saved at: ", csv_file))
    ...
    with_parameters_test_that("BreastCancerZNorm", test_name=paramGrid, test_params=paramGrid, code={
        for (iter in 1:1){
            ...
            csv_df <- cbind(actual = actual_classes,
                          ...
                          prob_0 = probabilities[,1],
                          prob_1 = probabilities[,2])
            csv_file <- file.path(dirname(dirname(getwd())),"predictions",paste0("pred_CARETreglogistic_LRridge_BreastCancerZNorm_",iter,".csv"))
            write.csv(x = csv_df,
                    file = csv_file,
                    row.names = FALSE)
            print(paste0("Predictions saved at: ", csv_file))
            ...
            csv_df <- cbind(actual = actual_classes_training_as_test,
                          ...
                          prob_0 = probabilities_training_as_test[,1],
                          prob_1 = probabilities_training_as_test[,2])
            csv_file <- file.path(dirname(dirname(getwd())),"predictions",paste0("pred_CARETreglogistic_LRridge_BreastCancerZNorm_TrainingAsTest_",iter,".csv"))
            write.csv(x = csv_df,
                    file = csv_file,
                    row.names = FALSE)
            print(paste0("Predictions saved at: ", csv_file))
    ...
    with_parameters_test_that("BreastCancerMinMaxNorm", test_name=paramGrid, test_params=paramGrid, code={
        for (iter in 1:1){
            ...
            csv_df <- cbind(actual = actual_classes,
                          ...
                          prob_0 = probabilities[,1],
                          prob_1 = probabilities[,2])
            csv_file <- file.path(dirname(dirname(getwd())),"predictions",paste0("pred_CARETreglogistic_LRridge_BreastCancerMinMaxNorm_",iter,".csv"))
            write.csv(x = csv_df,
                    file = csv_file,
                    row.names = FALSE)
            print(paste0("Predictions saved at: ", csv_file))
            ...
            csv_df <- cbind(actual = actual_classes_training_as_test,
                          ...
                          prob_0 = probabilities_training_as_test[,1],
                          prob_1 = probabilities_training_as_test[,2])
            csv_file <- file.path(dirname(dirname(getwd())),"predictions",paste0("pred_CARETreglogistic_LRridge_BreastCancerMinMaxNorm_TrainingAsTest_",iter,".csv"))
            write.csv(x = csv_df,
                    file = csv_file,
                    row.names = FALSE)
            print(paste0("Predictions saved at: ", csv_file))
    ...
    with_parameters_test_that("Wine", test_name=paramGrid, test_params=paramGrid, code={
        for (iter in 1:1){
            ...
            csv_df <- cbind(actual = actual_classes,
                          ...
                          prob_0 = probabilities[,1],
                          prob_1 = probabilities[,2])
            csv_file <- file.path(dirname(dirname(getwd())),"predictions",paste0("pred_CARETreglogistic_LRridge_Wine_",iter,".csv"))
            write.csv(x = csv_df,
                    file = csv_file,
                    row.names = FALSE)
            print(paste0("Predictions saved at: ", csv_file))
            ...
            csv_df <- cbind(actual = actual_classes_training_as_test,
                          ...
                          prob_0 = probabilities_training_as_test[,1],
                          prob_1 = probabilities_training_as_test[,2])
            csv_file <- file.path(dirname(dirname(getwd())),"predictions",paste0("pred_CARETreglogistic_LRridge_Wine_TrainingAsTest_",iter,".csv"))
            write.csv(x = csv_df,
                    file = csv_file,
                    row.names = FALSE)
            print(paste0("Predictions saved at: ", csv_file))
    ...
    with_parameters_test_that("WineZNorm", test_name=paramGrid, test_params=paramGrid, code={
        for (iter in 1:1){
            ...
            csv_df <- cbind(actual = actual_classes,
                          ...
                          prob_0 = probabilities[,1],
                          prob_1 = probabilities[,2])
            csv_file <- file.path(dirname(dirname(getwd())),"predictions",paste0("pred_CARETreglogistic_LRridge_WineZNorm_",iter,".csv"))
            write.csv(x = csv_df,
                    file = csv_file,
                    row.names = FALSE)
            print(paste0("Predictions saved at: ", csv_file))
            ...
            csv_df <- cbind(actual = actual_classes_training_as_test,
                          ...
                          prob_0 = probabilities_training_as_test[,1],
                          prob_1 = probabilities_training_as_test[,2])
            csv_file <- file.path(dirname(dirname(getwd())),"predictions",paste0("pred_CARETreglogistic_LRridge_WineZNorm_TrainingAsTest_",iter,".csv"))
            write.csv(x = csv_df,
                    file = csv_file,
                    row.names = FALSE)
            print(paste0("Predictions saved at: ", csv_file))
    ...
    with_parameters_test_that("WineMinMaxNorm", test_name=paramGrid, test_params=paramGrid, code={
        for (iter in 1:1){
            ...
            csv_df <- cbind(actual = actual_classes,
                          ...
                          prob_0 = probabilities[,1],
                          prob_1 = probabilities[,2])
            csv_file <- file.path(dirname(dirname(getwd())),"predictions",paste0("pred_CARETreglogistic_LRridge_WineMinMaxNorm_",iter,".csv"))
            write.csv(x = csv_df,
                    file = csv_file,
                    row.names = FALSE)
            print(paste0("Predictions saved at: ", csv_file))
            ...
            csv_df <- cbind(actual = actual_classes_training_as_test,
                          ...
                          prob_0 = probabilities_training_as_test[,1],
                          prob_1 = probabilities_training_as_test[,2])
            csv_file <- file.path(dirname(dirname(getwd())),"predictions",paste0("pred_CARETreglogistic_LRridge_WineMinMaxNorm_TrainingAsTest_",iter,".csv"))
            write.csv(x = csv_df,
                    file = csv_file,
                    row.names = FALSE)
            print(paste0("Predictions saved at: ", csv_file))
